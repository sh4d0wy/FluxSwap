...
import { ICreate3Deployer } from "@1inch/solidity-utils/contracts/interfaces/ICreate3Deployer.sol";
import { EscrowFactory } from "cross-chain-swap/EscrowFactory.sol";

contract DeployEscrowFactoryWithLOP is Script {
    uint32 public constant RESCUE_DELAY = 691200; // 8 days
    bytes32 public constant CROSSCHAIN_SALT = keccak256("1inch EscrowFactory");
    
    // You can override this address with your deployed LOP
    address public constant LOP = 0x111111125421cA6dc452d289314280a0f8842A65; // Default 1inch LOP
    ...
    function setUp() public {
        ...
        FEE_TOKEN[11155111] = 0x68194a729C2450ad26072b3D33ADaCbcef39D574; // Sepolia (DAI)
    ...
    function run() external {
        ...
        // Allow override of LOP address via environment variable
        address lopAddress = vm.envOr("LOP_ADDRESS", LOP);

        if (feeToken == address(0)) {
            revert("Fee token not configured for this chain");
        }

        console.log("Deploying EscrowFactory with:");
        ...
        address escrowFactory = CREATE3_DEPLOYER.deploy(
            ...
            abi.encodePacked(
                ...
                abi.encode(lopAddress, feeToken, ACCESS_TOKEN, feeBankOwner, RESCUE_DELAY, RESCUE_DELAY)
            )
        );
        vm.stopBroadcast();
        ...
        string memory deploymentInfo = string.concat(
            "EscrowFactory deployed at: ",
            vm.toString(escrowFactory),
            "\nChain ID: ",
        ...
        // vm.writeFile("deployment-escrow-factory.txt", deploymentInfo); // Commented out due to Foundry restrictions
    }
} 